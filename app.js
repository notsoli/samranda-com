...
function initImage(img) {
    ...
    let y_pos = project.getBoundingClientRect().y + document.documentElement.scrollTop

    x_pos = (Math.round(Math.random())) ? -img.width / 2 : window.innerWidth + img.width / 2
    y_pos += (Math.random() * 50) - 25

    img.style.left = x_pos + "px"
    img.style.top = y_pos + "px"

    const x_spd = ((Math.round(Math.random()) * 2) - 1) * ((Math.random() * 0.5) + 0.5)
...
function render() {
    ...
    for (const img of images) {
        ...
        if (x < -img.width / 2 || x > window.innerWidth + img.width / 2) initImage(img)
    }

    if (frame % 240 == 0) {
        const project = images[highlight].parentElement
        project.classList.add("hover")
        setTimeout(() => { project.classList.remove("hover") }, 3000)
