...
function initImage(img) {
    ...
    let y_pos =
        project.getBoundingClientRect().y + document.documentElement.scrollTop;

    const x_pos = Math.round(Math.random())
        ? -img.width / 2
    ...
    const x_spd =
        (Math.round(Math.random()) * 2 - 1) * (Math.random() * 0.5 + 0.5);
...
function render() {
    ...
    if (frame % 240 == 0) {
        const project = images[highlight % images.length].parentElement;
        project.classList.add("hover");
        setTimeout(() => {
            project.classList.remove("hover");
        }, 3000);
        highlight += 3 + Math.floor(Math.random() * 3);
    }
